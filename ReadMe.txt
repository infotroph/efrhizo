...
data: cleaned-up, finished, most authoritatize versions of datasets. Everything* here is generated by some scripted process, NEVER by hand-editing. 
	*EXCEPTION: as of 2016-01-09, subdirectory `2012` still contains hand-edited Excel spreadsheets: 3 of the 6 2012 tracing log files live here until I convert them to CSVs & add their untraceable images to the censored frames list. There are also two spreadsheets used to work out imagename corrections for 2012, which will probably live in `notes/` someday but maybe they need to be in a more useful format first.
...
stan: Scripts for hierarchical Bayesian inference on how root volume differs between crops and over time, written in the probabalistic programming language Stan. Also contains R and Bash scripts to handle the process of running the models on the IGB computing cluster; 
	N.B. 1: These models are NOT yet run automatically by Make, and as of 2016-01-09 the output isn't yet saved in the Git repositroy. Be sure to rerun the models after any update to the data.
